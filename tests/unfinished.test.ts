...
describe("unfinished tasks", () => {
  it("finds tasks that fell off the lists and lets them come back to today", async () => {
    ...
    const oldProgressed = await add(mon, "Old progressed task");
    const done = await add(mon, "Pay rent");
    const carried = await add(mon, "Untouched task");
    await setEntryStatus(mon, progressed.entry!.id, "progressed"); // progressed one-offs now carry to tomorrow
    await setEntryStatus(mon, oldProgressed.entry!.id, "progressed");
    ...
    await rolloverIfNeeded(tue); // carries the untouched and the progressed ones onto today
    expect((await q("select 1 from day_entries where task_id = $1 and date = '2026-09-22'", [progressed.task.id])).length).toBe(1);
    // a task that fell off before the rule changed (or was removed from today) is still found
    await q("delete from day_entries where task_id = $1 and date = '2026-09-22'", [oldProgressed.task.id]);

    const list = await unfinishedTasks(tue.today);
    expect(list.map((t) => t.title).sort()).toEqual(["Call bank", "Old progressed task"]);
    const p = list.find((t) => t.title === "Old progressed task")!;
    ...
    expect(await bringToToday(tue, [oldProgressed.task.id], true)).toBe(1);
